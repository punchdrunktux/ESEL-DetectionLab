...
fix_eth1_static_ip() {
  ...
  echo -e 'interface "eth1" {
    ...
    send dhcp-requested-address 10.10.20.27;
  }' >>/etc/dhcp/dhclient.conf
  netplan apply
  # Fix eth1 if the IP isn't set correctly
  ETH1_IP=$(ip -4 addr show eth1 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
  if [ "$ETH1_IP" != "10.10.20.27" ]; then
    ...
    if [ "$ETH1_IP" == "10.10.20.27" ]; then
      echo "[$(date +%H:%M:%S)]: The static IP has been fixed and set to 10.10.20.27"
    else
...
install_splunk() {
  ...
  if [ -f "/opt/splunk/bin/splunk" ]; then
    ...
    /opt/splunk/bin/splunk install app /vagrant/resources/spluxnk_server/punchcard-custom-visualization_130.tgz -auth 'admin:changeme'
...
import_osquery_config_into_fleet() {
  ...
  fleetctl config set --address https://10.10.20.27:8412
